...
# by Xingpeng.Li
# run command: include fileName.mod; e.g. include MG_IC_e1.mod;
...
# Parameters
param TL_Limit = 100; # Tie-line limit
...
minimize obj: sum{g in GEN, t in PERIOD} (gen_OpCost[g]*Pg[g,t] + gen_NlCost[g]*u[g,t] + gen_SuCost[g]*v[g,t]) + sum{t in PERIOD}Pgrid[t]*GridPrice[t];
             
# Define constraints
subject to PowerBalance {t in PERIOD}: sum{g in GEN} Pg[g,t] = Time_TotalPd[t] - SolarP[t] - Pgrid[t] + sum{e in STORAGE}(P_c_ES[e,t] - P_d_ES[e,t]);
