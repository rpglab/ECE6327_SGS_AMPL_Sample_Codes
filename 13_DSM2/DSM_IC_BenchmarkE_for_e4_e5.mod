...
# by Xingpeng.Li
# run command: include fileName.mod; e.g. include DSM_IC_BenchmarkE_for_e4_e5.mod; 
...
param DSM_Cost = 25;           # 25 for benchmarking example 4; 10 for benchmarking example 5. 
...
minimize obj: sum{g in GEN, t in PERIOD} (gen_OpCost[g]*Pg[g,t] + gen_NlCost[g]*u[g,t] + gen_SuCost[g]*v[g,t])
               + sum{t in PERIOD: t<nT}DSM_d[t]*DSM_Cost;
...
# Load data
data DSM_IC_BenchmarkE_for_e4_e5.txt;

fix {g in GEN, t in PERIOD} u[g,t] := 1; 
